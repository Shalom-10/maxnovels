...
function moderate(name, image, id, self) {
    ...
    modal.querySelector('input[name = "modal-input"]').value = name;
    modal.querySelector('img').src = `/images/collections/${image}`;

    modal.querySelector(".btn-left").setAttribute('onclick', `flash_card_trigger('you are about to update this category, are you sure you want to?', '', 'update(${id});');`);
    modal.querySelector(".btn-right").setAttribute('onclick', `flash_card_trigger('you are about to delete this category, are you sure you want to?', '', 'delete_collection(${id});');`);
...
function update(id) {
    ...
    collection_data.append('name', document.querySelector('input[name = "modal-input"]').value);
    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            current_column.parentNode.parentNode.querySelector('.item-name').innerHTML = data.name;
